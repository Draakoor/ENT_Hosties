...
void MutePrisoners_OnPluginStart()
{
	...
	gH_Cvar_MuteImmune = CreateConVar("sm_hosties_mute_immune", "z", "Admin flags which are immune from getting muted: 0 - nobody, flag values: abcdefghijklmnopqrst");	
...
stock void CalImmun()
{
	...
	if (StrContains("abcdefghijklmnopqrst", flag) != -1)
	{
		GetFlagInt(flag);
		gAdmFlags_MuteImmunity = StringToInt(flag);
	}
	else if (StrEqual("0", flag))
...
stock void MuteTs()
{
	...
	{
		if (IsClientInGame(i) && IsPlayerAlive(i) && GetClientTeam(i) == CS_TEAM_T) // if player is in game and alive
		{
...
public Action MutePrisoners_PlayerDeath(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (gAdmFlags_MuteImmunity == 0 || !Client_HasAdminFlags(victim, gAdmFlags_MuteImmunity))
	{
...
public Action MutePrisoners_RoundStart(Event event, const char[] name, bool dontBroadcast)
{
	if (gH_Cvar_MuteStatus.IntValue == 1 || gH_Cvar_MuteStatus.IntValue == 3)
	{
		...
		else
		{
			...
			{
				if (IsValidClient(idx) && (GetClientTeam(idx) == CS_TEAM_T) && !Client_HasAdminFlags(idx, gAdmFlags_MuteImmunity))
				{
					MutePlayer(idx);
				}
			}
